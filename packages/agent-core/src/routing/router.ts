...
function resolveDefaultAgentId(rawDefaultAgentId: string | undefined): string {
...
function readNamedAgentDirective(trimmedInput: string): ResolveAgentRouteResult | null {
  ...
  const rawAgentId =
    spaceIndex === -1 ? remainder : remainder.slice(0, Math.max(0, spaceIndex));
...
function readResearchDirective(trimmedInput: string): ResolveAgentRouteResult | null {
