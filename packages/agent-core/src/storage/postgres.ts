...
const schemaInitByClient = new WeakMap<PostgresStorageClient, Promise<void>>();
...
function scoreMemoryRecord(record: { text: string; tags: string[] }, queryTerms: string[]): number {
...
function normalizeMcpServerInput(input: McpServerInput): McpServerConfig {
  return {
    id: input.id.trim(),
    url: input.url.trim(),
...
export async function ensurePostgresSchema(client: PostgresStorageClient): Promise<void> {
...
export class PostgresSessionStore extends SessionStore {
  ...
  public constructor(client: PostgresStorageClient) {
    super(".");
    this.client = client;
    this.ready = ensurePostgresSchema(client);
  }

  public override async load(sessionKey: string): Promise<Message[]> {
    await this.ready;
    ...
    return rows
      .map((row) => row.message)
      .filter((value): value is Message => isMessage(value));
  }

  public override async append(sessionKey: string, message: Message): Promise<void> {
    await this.ready;
    await this.client.db.transaction(async (tx) => {
      const rows = await tx
        .select({ maxIndex: sql<number>`coalesce(max(${sessionMessages.messageIndex}), -1)` })
        .from(sessionMessages)
        .where(eq(sessionMessages.sessionKey, sessionKey));
  ...
  public override async save(sessionKey: string, messages: Message[]): Promise<void> {
    await this.ready;
    await this.client.db.transaction(async (tx) => {
...
export class PostgresMemoryStore extends MemoryStore {
  private readonly client: PostgresStorageClient;
  protected override readonly embeddingProvider: MemoryEmbeddingProvider | undefined;

  private readonly ready: Promise<void>;

  public constructor(client: PostgresStorageClient, embeddingProvider?: MemoryEmbeddingProvider) {
  ...
  public override async saveMemory(input: SaveMemoryInput): Promise<SavedMemory> {
    await this.ready;
  ...
  public override async searchMemory(input: MemorySearchInput): Promise<MemorySearchResult[]> {
    await this.ready;
  ...
  public override async backfillEmbeddings(limit = 100): Promise<{ indexed: number; skipped: number }> {
    await this.ready;
...
export class PostgresMcpServerStore extends McpServerStore {
  ...
  public override async listServers(): Promise<McpServerConfig[]> {
    await this.ready;
    const rows = await this.client.db
  ...
  public override async saveServers(servers: McpServerConfig[]): Promise<void> {
    await this.ready;
    await this.client.db.transaction(async (tx) => {
  ...
  public override async upsertServer(input: McpServerInput): Promise<McpServerConfig[]> {
    await this.ready;
  ...
  public override async removeServer(serverId: string): Promise<McpServerConfig[]> {
    await this.ready;
...
export class PostgresGatewayIdempotencyStore implements GatewayIdempotencyStore {
  private readonly client: PostgresStorageClient;
  ...
  public constructor(client: PostgresStorageClient, ttlMs = 24 * 60 * 60 * 1000) {
    this.client = client;
    this.ttlMs = ttlMs;
    this.ready = ensurePostgresSchema(client);
  }
  ...
  public async get(sessionKey: string, idempotencyKey: string): Promise<IdempotencyEntry | null> {
    ...
    const rows = await this.client.db
      ...
      .where(
        and(
          eq(gatewayIdempotency.sessionKey, sessionKey),
          eq(gatewayIdempotency.idempotencyKey, idempotencyKey),
        ),
      )
      .limit(1);
    ...
    return {
      ...
      result: row.result as unknown as HandleMessageResult,
...
export class PostgresSessionLockManager implements SessionLockManager {
  private readonly client: PostgresStorageClient;
  ...
  public async runExclusive<T>(sessionKey: string, task: () => Promise<T>): Promise<T> {
    ...
    return this.client.db.transaction(async (tx) => {
      await tx
        ...
        .onConflictDoNothing();
      await tx.execute(
        sql`select session_key from gateway_session_locks where session_key = ${sessionKey} for update`,
      );
      return task();
    });
  }
