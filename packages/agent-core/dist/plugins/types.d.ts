...
export declare const PLUGIN_CAPABILITIES: readonly ["tools", "prompt", "turn", "tool_intercept"];
export type PluginCapability = (typeof PLUGIN_CAPABILITIES)[number];
export declare const PLUGIN_HOOK_NAMES: readonly ["before_prompt_compose", "after_prompt_compose", "before_turn", "after_turn", "before_tool_call", "after_tool_call"];
...
export declare const PLUGIN_PERMISSIONS: readonly ["memory.read", "memory.write", "session.read", "session.write", "channel.send", "channel.receive", "network.http", "filesystem.read", "filesystem.write"];
...
export interface PluginHooks {
    ...
    beforeTurn?(event: PluginBeforeTurnEvent): Promise<Partial<Pick<PluginBeforeTurnEvent, "text" | "systemPrompt">> | void> | Partial<Pick<PluginBeforeTurnEvent, "text" | "systemPrompt">> | void;
    afterTurn?(event: PluginAfterTurnEvent): Promise<void> | void;
    beforeToolCall?(event: PluginBeforeToolCallEvent): Promise<Partial<Pick<PluginBeforeToolCallEvent, "input">> | void> | Partial<Pick<PluginBeforeToolCallEvent, "input">> | void;
    afterToolCall?(event: PluginAfterToolCallEvent): Promise<Partial<Pick<PluginAfterToolCallEvent, "output">> | void> | Partial<Pick<PluginAfterToolCallEvent, "output">> | void;
}
...
export interface PluginLifecycle {
    ...
    onHealthCheck?(context: PluginLifecycleContext): Promise<{
        healthy: boolean;
        details?: string;
    } | void> | {
        healthy: boolean;
...
export declare const PLUGIN_EVENT_NAMES: readonly ["plugin.loaded", "plugin.failed", "plugin.disabled", "plugin.hook.started", "plugin.hook.completed", "plugin.hook.timed_out", "plugin.tool.executed", "plugin.permission.denied"];
