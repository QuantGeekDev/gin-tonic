...
function outputFormatToContentType(format) {
...
export function resolveTtsConfigFromEnv(env) {
    const providerRaw = env.JIHN_TTS_PROVIDER?.trim().toLowerCase();
    const provider = providerRaw === "none" || providerRaw === undefined || providerRaw.length === 0
        ? "none"
        : providerRaw === "elevenlabs"
            ? "elevenlabs"
            : (() => {
                throw new Error(`Unsupported JIHN_TTS_PROVIDER '${env.JIHN_TTS_PROVIDER}'. Allowed: none, elevenlabs`);
            })();
...
export function createTtsProviderFromEnv(env) {
