import { resolveTtsConfigFromEnv } from "../tts/elevenlabs.js";
...
function normalizeChannelEnvPrefix(channelId) {
...
function parseMode(raw) {
    ...
    throw new Error(`Unsupported TTS mode '${raw}'. Allowed: off, text_and_voice, voice_only`);
}
function parseMaxChars(raw) {
    ...
    if (parsed < MIN_MAX_CHARS || parsed > MAX_MAX_CHARS) {
        throw new Error(`Invalid TTS max chars '${raw}'. Allowed range: ${MIN_MAX_CHARS}-${MAX_MAX_CHARS}.`);
    }
    return parsed;
}
export function resolveChannelTtsPolicyFromEnv(params) {
    ...
    const maxChars = parseMaxChars(env[`${prefix}MAX_CHARS`]) ??
    ...
    const outputFormat = normalizeNonEmpty(env[`${prefix}OUTPUT_FORMAT`]) ?? providerConfig.outputFormat;
...
//# sourceMappingURL=tts-policy.js.map