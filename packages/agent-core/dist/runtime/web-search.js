...
function maybeUnwrapDuckDuckGoRedirect(rawHref) {
    try {
...
function assertRemoteHttpUrl(value, allowPrivateHosts) {
    let url;
    try {
        url = new URL(value);
    }
...
async function searchWithWikipediaFallback(fetchFn, query, limit) {
...
export function createDefaultWebSearchClient(options = {}) {
    ...
    return {
        async search(params) {
            ...
            const response = await fetchWithTimeout(fetchFn, searchUrl, {
                headers: {
        ...
        async fetchPage(params) {
            ...
            const response = await fetchWithTimeout(fetchFn, target.toString(), {
                redirect: "follow",
                headers: {
                    accept: "text/html,application/xhtml+xml",
                    "user-agent": userAgent,
            ...
            const title = titleMatch && titleMatch[1] ? normalizeWhitespace(decodeHtmlEntities(stripTags(titleMatch[1]))) : null;
